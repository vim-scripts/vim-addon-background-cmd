...
" run a task in background.. Testing has to be done on Windows
...
" start region 'file <dotvim>/autoload/vl/dev/errorformat.vim'
" "|fld   description : Some errorformats for different applications
" "|fld   keywords : <+ this is used to index / group scripts ?+> 
" "|fld   initial author : Marc Weber marco-oweber@gmx.de
" "|fld   mantainer : author
" "|fld   started on : 2007 Nov 01 23:20:26
" "|fld   version: 0.0
" "|fld   dependencies: <+ vim-python, vim-perl, unix-tool sed, ... +>
" "|fld   contributors : <+credits to+>
" "|fld   tested on os : <+credits to+>
...
" "|set   <description of setting(s)>
" "|      "description
" "let s:<+setting name+>=vl#lib#vimscript#scriptsettings#Load('vl.dev.errorformat.<+setting_name+>',<+ default +>)
" "|
" "|
" "|hist <+ historical information. (Which changes have been made ?) +>
" "|
...
" "|rm roadmap (what to do, where to go?)
" 
" " I keep all errorformats here to look some things up more easily.. This way 
" 
" function! vl#dev#errorformat#ErrorFormat(application, ...)
"   " flavour will become something like cygwin/ vim /etc There might be
"   " differences someday? 
"   exec vl#lib#brief#args#GetOptionalArg('flavour',string('default'))
"   " by executable name
"   "
"   " js : spidermonkey js
"   " /tmp/test.js:1: SyntaxError: missing ; before statement:
"   " /tmp/test.js:1: (oanetuho))
"   " /tmp/test.js:1: ..........^
"   let formats = {
"         \ 'js' : [ 
"               \    '%E%f:%l: %m'
...
"             \ , "%-G%.%#"
"             \ ],
"       \ 'python' : [ '%A  File "%f", line %l, %m' ],
"       \ 'nix' : [ "%m, at `%f':%l:%c"
"             \ , "%m at `%f', line %l:"
"             \ , "error: %m, in `%f'" ]
"     \ }
"     return formats[a:application]
" 
" 
"     "" python
" 	"\%C\ \ \ \ %.%#,
...
" endfunction
" 
" function! vl#dev#errorformat#EscapePattern(p)
"   return substitute(substitute(substitute(substitute(a:p
"      \ ,' ', '\\ ','g')
"      \ ,'"','\\"','g')
"      \ ,'|','\\|','g')
"      \ ,',', '\\\\\,','g')
" endfunction
" 
" function! vl#dev#errorformat#ErrorFormatSet(application)
"   "echoe 'set efm='.substitute(substitute(vl#dev#errorformat#ErrorFormat(a:application),' ', '\ ','g'),'|','\|','g')
"   "TODO escape , 
"   exec 'silent! set efm='.join(vl#lib#listdict#list#MapCopy(vl#dev#errorformat#ErrorFormat(a:application), 'vl#dev#errorformat#EscapePattern(Val)'),',')
" endfunction
" end region 'file <dotvim>/autoload/vl/dev/errorformat.vim'
...
" start region 'file <dotvim>/autoload/vl/lib/quickfix/runtaskinbackground.vim'
" " purpose: run commands in background and continue with your work
" " version 0.2 (updated)
" " os: 
" "   linux with bash or python enabled, 
" "   windows no background (using python I'm getting a invalid handle error ..
" "   using sh I hadn't time to make it call cypath)
" "
" "   when using python the output to stderr is appended to stdout
" "   in bash/ cmd &> is used for redirection
" "   all running processes are listed in g:processes
" "   all finished processes are listed in g:finished_processes
" 
" " usage: see RunInBGQFAW
" " eg call vl#lib#quickfix#runtaskinbackground#RunInBGQFAW(['make','all'])
" " You cann add the command
" " (BG = background QF = quick fix  AW = autowrite (emulated))
" " BG BGQF BGQFAW using the function /scrollown to find it
" "
" "
" " As this scirpt is still in developement I don't add the common
" " if exists('g:runinbackground_sourced') 
" "   finish
" " endif
" " let g:runinbackground_sourced=1
" " yet
" "
" "
" " you can switch off some python/ shell/ make etc by setting failX to 1 (see
" " debugging options below)
" "
" " Thins to think about:
" " 'shellcmdflag' , 'shell' (how to use sh on windows)?
" " (or can cmd be used to run a commmand in background ?)
" " maybe use cmdow executable to hide cmd window on windows
" " how, when to mix stderr, stdout? Right now most ways try to get both
" 
" 
...
" function! s:ClientServer()
"   return has('clientserver') && has('gui_running')
" endfun
" 
" "called when a process has been started (get the process info by using
" "g:processes[vimPID]
" let s:start=vl#lib#vimscript#scriptsettings#Load(
"     \ 'vl.lib.quickfix.runtaskinbackground.start', 'exec "call vl#lib#ide#logging#Log(''process''.vimPID.'' has started'')"' )
" "called when a process has stopped
" let s:stop=vl#lib#vimscript#scriptsettings#Load(
"       \ 'vl.lib.quickfix.runtaskinbackground.stop', 'exec "call vl#lib#ide#logging#Log(''process''.vimPID.'' has finished with status ''.a:info)"' )
" " called when at least one process has started
" let s:any_running=vl#lib#vimscript#scriptsettings#Load(
"     \ 'vl.lib.quickfix.runtaskinbackground.any_running', 'silent! let g:oldc = g:colors_name | colorscheme morning' )
" " called when no longer any process is running
" let s:none_running=vl#lib#vimscript#scriptsettings#Load(
"     \ 'vl.lib.quickfix.runtaskinbackground.none_running', 'silent! exec "colorscheme ".g:oldc | silent! unlet g:oldc' )
" 
" " specify your sh location here
" let s:sh_locations=vl#lib#vimscript#scriptsettings#Load(
"     \ 'vl.lib.quickfix.runtaskinbackground', ['sh','c:/cygwin/bin/sh'] )
" 
" " get sh location
" let [s:sh_availible, s:sh_executable] = vl#lib#brief#handler#HandleList( s:sh_locations
" 		\ , vl#dev#vimscript#vimfile#Function('vl#lib#brief#handler#IsExecutableHandlerfunc'))
" 
" silent! let s:normal_colorscheme=g:colors_name
" 
" " debugging options
" let s:failSh = 0
" let s:failPython = 0
" " use makeprg and make command to execute (prefered because it doesn't show up
" " the cmd window on windows
" let s:failMake = 0
" let s:failExclamation = 0
" 
" " try to get vim with clientserver 
" let [s:success, s:vim] = vl#lib#introspection#VimFilepath()
" if !s:success 
"   if exists('g:vim_filepath')
"     let s:vim = g:vim_filepath
"   elseif executable('gvim')
"     " gvim has clientserver more likely than vim
"     let s:vim = 'gvim'
"   else
"     let s:vim = ''
"   endif
...
" fun! s:Start(vimPID)
"   let vimPID = a:vimPID
"   "call vl#lib#ide#logging#Log(string(s:running))
"   exec s:start
"   let s:running = s:running + 1
"   if s:running == 1
"     "call vl#lib#ide#logging#Log(s:any_running)
"     exec s:any_running
"   endif
" endfun
" 
" " info is status or message
" fun! s:Stop(vimPID, info)
"   "call vl#lib#ide#logging#Log(string(g:processes[a:vimPID]).' finished')
"   let vimPID = a:vimPID
"   let s:running = s:running - 1
"   if s:running == 0
"     exec s:none_running
...
" endfun
" :
" "func add commands to be used by you
" 
" fun! vl#lib#quickfix#runtaskinbackground#ProcessFinished(vimPID,status)
"   let vimPID = a:vimPID " used for exec commond only (se RunInBGQFAW)
...
"   endif
"   call s:Stop(vimPID, a:status)
"   let g:finished_processes[a:vimPID] = remove(g:processes,a:vimPID)
" endfun
" 
" fun! vl#lib#quickfix#runtaskinbackground#ProcessHasentBeenStartedBecause(vimPID,message)
"   echomsg 'vimPID '.a:vimPID." hasn't started because ".a:message
"   let vimPID = a:vimPID " used for exec commond only (see RunInBGQFAW)
" 
"   if exists('g:processes["'.a:vimPID.'"]["vimcmd"]')
"     echomsg g:processes[a:vimPID]["vimcmd"]
"     silent! exec g:processes[a:vimPID]["vimcmd"]
"   endif
"   let entry = g:processes[a:vimPID]
"   if !exists('g:finished_processes')
"     let g:finished_processes={}
"   endif
"   let g:processes['failed_running'] = 1
"   call s:Stop(vimPID, a:message)
"   let g:finished_processes[a:vimPID] = remove(g:processes,a:vimPID)
" endfun
" 
" " from now on we know which process to kill
" fun! vl#lib#quickfix#runtaskinbackground#NotifyProcessId(vimProcessId,pid)
"   "call vl#lib#ide#logging#Log('pid of vimProcessId is '.a:vimProcessId.' '.a:pid)
"   exec s:Start(a:vimProcessId) 
"   let g:processes[a:vimProcessId]['pid']=a:pid
" endfun
" 
" function! vl#lib#quickfix#runtaskinbackground#SplitCommand(s)
"     "TODO handle proper escaping 
"     split(a:1, '\s\+')
" endfunction
" 
" " RunInBG(['grep', '-r', '.'])
" fun! vl#lib#quickfix#runtaskinbackground#RunInBG(cmd_list)
"   let vimPID = vl#lib#quickfix#runtaskinbackground#NewVimProcessId(a:cmd_list)
"   call s:RunCommandInBackground(vimPID)
" endfun
" 
" " the same ase RunInBG but loads the std/err output into quickfix buffer
" " optional:
" " options: { 'loadErrors' : 'cfile' (default)
" "          , 'onFinish' : ' echo "exit status has been".a:status '
" "          , 'efm' : 'php'
" "          }
" " onFinish can also be a list
" fun! vl#lib#quickfix#runtaskinbackground#RunInBGQF(cmd, ...)
"   exec vl#lib#brief#args#GetOptionalArg('options',string({}))
"   if has_key(options, 'efm')
"     call vl#dev#errorformat#ErrorFormatSet(options['efm'])
"   endif
"   let loadErrors = get(options, 'loadErrorCmd', 'cfile')
"   let vimCmdAfter = "silent! exec 'silent! ".loadErrors." '.g:processes[vimPID]['stdout']"
"   if has_key(options, 'onFinish')
"     " not all commands will let you add another |, thus the MapCopy adding exec
"     let vimCmdAfter = vimCmdAfter . ' | '.join(vl#lib#listdict#list#MapCopy(
"           \   vl#lib#listdict#list#ToList(options['onFinish'])
"           \ , '"exec ".string(Val)' ), "|")
"   endif
"   let g:vimCmdAfter = vimCmdAfter
"   let vimPID = vl#lib#quickfix#runtaskinbackground#NewVimProcessId(a:cmd, vimCmdAfter)
"   call s:RunCommandInBackground(vimPID)
" endfun
" 
" " the same as RunInBGQF but runs autowrite
" fun! vl#lib#quickfix#runtaskinbackground#RunInBGQFAW(cmd_list, ...)
"   exec vl#lib#brief#args#GetOptionalArg('options',string({}))
"   " emulate autowrite
"   let buf_nr = bufnr('%')
"   bufdo if &autowrite && len(expand('%')) > 0 | update | endif
"   silent! exec 'buffer '.buf_nr
" 
"   if (a:0 == 1)
"     call vl#dev#errorformat#ErrorFormatSet(options['efm'])
"   endif
" 
"   let vimCmdAfter = "silent! exec 'silent! cfile '.g:processes[vimPID]['stdout']"
"   if has_key(options, 'onFinish')
"     " not all commands will let you add another |, thus the MapCopy adding exec
"     let vimCmdAfter = vimCmdAfter . ' | '.join(vl#lib#listdict#list#MapCopy(
"           \   vl#lib#listdict#list#ToList(options['onFinish'])
"           \ , '"exec ".string(Val)' ), "|")
"   endif
"   let vimPID = vl#lib#quickfix#runtaskinbackground#NewVimProcessId(a:cmd_list, vimCmdAfter)
"   call s:RunCommandInBackground(vimPID)
" endfun
" 
" fun! vl#lib#quickfix#runtaskinbackground#ShExprCallingVim(vimcmd)
"     return s:vim." --servername \"".v:servername."\" --remote-send \"<esc>:".a:vimcmd."<cr>\"" 
" endfun
" 
" function! vl#lib#quickfix#runtaskinbackground#KillAllRunningTasks()
"   for key in keys(g:processes)
"     let i = g:processes[key]
"     if exists("i['pid']")
"       let cmd = 'kill '.i['pid']
"       call system(cmd)
"     else
"       echoe "couldn't call kill for all running taskes, some haven't been assigned a pid yet"
"     endif
"     call vl#lib#quickfix#runtaskinbackground#ProcessFinished(key, 'finished kill attempt by vim')
"   endfor
" endfunction
" command! KillAllRunningTasks :call vl#lib#quickfix#runtaskinbackground#KillAllRunningTasks()<cr>
" 
...
" fun! vl#lib#quickfix#runtaskinbackground#NewVimProcessId(cmd,...)
...
"   let g:processes[vimPID]['cmd'] = a:cmd
"   if a:0>0
"     let g:processes[vimPID]['vimcmd']=a:1
"   endif
"   let g:processes[vimPID]['stdout'] = tempname()
"   return vimPID
" endfun
" 
" " does some escaping
" fun! vl#lib#quickfix#runtaskinbackground#CommandToSh(cmd)
"   let result = []
"   for i in a:cmd
"     "let i = substitute(i,' ','\ ','g')
"     "let i = substitute(i,'"','\" ','g')
"     "let result += '"'+[i]+'"'
"     call add(result, string(i))
"   endfor
"   return join(result,' ')
" endfun
" 
" " try all handlers .. (is called by RunInBG* )
" fun! s:RunCommandInBackground(vimPID)
"   call vl#lib#ide#logging#Log(string(g:processes[a:vimPID]['cmd']))
"   call vl#lib#ide#logging#Log("running command (quotes missing in log): \n".join(g:processes[a:vimPID]['cmd'],' '))
"   let [success, value] =  vl#lib#brief#handler#Handle(
"      \ [ function('vl#lib#quickfix#runtaskinbackground#RunProcessInPythonThread')
"      \ , function('vl#lib#quickfix#runtaskinbackground#RunProcessUsingSh')
"      \ , function('vl#lib#quickfix#runtaskinbackground#RunProcessUsingMake')
"      \ , function('vl#lib#quickfix#runtaskinbackground#RunProcessUsingSystem')
"      \ ], a:vimPID  )
"   let g:result = value
"   " success = 1 does'nt mean the process has finished yet but that it has been
"   " started successfully
"   " Stop must be called by the handler
"   if !success
"     " Exclamation can't fail
"     let log_cmd_info = string(g:processes[a:vimPID]['cmd'])
"     "call vl#lib#ide#logging#Log(vl#lib#listdict#list#Concat(
"     "                            \ [ "running process ".log_cmd_info." failed (neither sh, python, ! was succesful)\n" ] , value)
"     "                          \ , 10 )
"   else
"     "call vl#lib#ide#logging#Log(value)
"   endif
" endfun
" 
" " runs the cmd using sh -c "cmd &"
" fun! vl#lib#quickfix#runtaskinbackground#RunProcessUsingSh(vimPID)
"   if !s:ClientServer()
"     return [0, "no client server"]
"   endif
"   if s:failSh
"     return [0, "debug aborted"]
"   endif
"   if !s:sh_availible 
"     return [0, "no sh found"]
"   endif
"   if !has('clientserver')
"     return [0, "no client server version"]
"   endif
"   if !executable(s:vim)
"     return [0, "this vim not found. set g:vim_filepath"]
"   endif
"   if has('win16') || has('win32') || has('win64')
"     return [0, "run task in background using sh background processes not yet supported"]
"   endif
"   call vl#lib#ide#logging#Log("executing command using sh")
"   " let vim now which pid it is (bash $$ substitution
"   let shcmd = vl#lib#quickfix#runtaskinbackground#ShExprCallingVim('call vl#lib#quickfix#runtaskinbackground#NotifyProcessId('.a:vimPID.',"$$")').';'
"   " execute the command! :
"    if executable('cygpath')
"      let stdoutfile = system('c:/cygwin/bin/cygpath -u '.g:processes[a:vimPID]['stdout'])
"    else
"      let stdoutfile = g:processes[a:vimPID]['stdout']
"    endif
"   "call vl#lib#ide#logging#Log(stdoutfile.'  '.s:sh_executable)
"   let shcmd .= vl#lib#quickfix#runtaskinbackground#CommandToSh(g:processes[a:vimPID]['cmd']).' &> '.stdoutfile.'; '
"   " notify vim that the command! has finished
"   let shcmd .= vl#lib#quickfix#runtaskinbackground#ShExprCallingVim("silent! call vl#lib#quickfix#runtaskinbackground#ProcessFinished('".a:vimPID."',$?)").';'
"   let cmdtoexecute='sh -c "'.substitute(shcmd,'"','\\"','g').'" &'
"   let g:processes[a:vimPID]['shcmd'] = cmdtoexecute
"   call system(cmdtoexecute)
"   return [1,'sh background job started']
" endfun
" 
" " runs the cmd using vims makeprg setting and make cmd
" fun! vl#lib#quickfix#runtaskinbackground#RunProcessUsingMake(vimPID)
"   " not yet supported fully
"   if s:failMake || 1
"     return [0,"running useing make debug aborted"]
"   endif
"   call vl#lib#ide#logging#Log("executing command using make")
"   let old_makeprg = &makeprg
"   let cmd = copy(g:processes[a:vimPID]['cmd'])
"   let &makeprg = vl#lib#listdict#list#Pop(cmd)
"   if has('win16') || has('win32') || has('win64')
"     let &makeprg = substitute(&makeprg,'^./','','')
"   endif
"   let cmd_opts = join(cmd, " ").' > "'.g:processes[a:vimPID]["stdout"].'" 2>&1'
" 
"   call s:Start(a:vimPID)
"   "call vl#lib#ide#logging#Log('running :make driven :'.&makeprg+' '+cmd_opts)
"   let g:g = cmd_opts
"   exec 'make '.cmd_opts
"   call s:Stop(a:vimPID, '?')
"   return [1, "finished make"]
" endfun
" 
" " runs the cmd using vims :! command
" fun! vl#lib#quickfix#runtaskinbackground#RunProcessUsingSystem(vimPID)
"   if s:failExclamation
"     return [0,"debug aborted"]
"   endif
"   call vl#lib#ide#logging#Log("executing command using sytem()")
"   " There is no possibility to check exit status
"   let cmd = copy(g:processes[a:vimPID]['cmd'])
"   call map(cmd, '''"''.substitute(v:val, ''\\'',''\\\\'',''g'').''"''')
"   let cmd_str = join(cmd, " ").' > "'.g:processes[a:vimPID]["stdout"].'" 2>&1'
"   if has('win16') || has('win32') || has('win64')
"     " remove escaping of first executable ""
"     let cmd_str = substitute(cmd_str,'"','','')
"     let cmd_str = substitute(cmd_str,'"','','')
"     " no "" around the first excutable when using cmd from windows !
"     let cmd_str = substitute(cmd_str,'^./','','')
"   endif
"   call s:Start(a:vimPID)
"   "exec '!'.cmd_str
"   echo cmd_str
"   " call system('{ '.cmd_str.' ;} 2>&1')
"   call system(cmd_str)
"   call s:Stop(a:vimPID, v:shell_error)
"   call vl#lib#quickfix#runtaskinbackground#ProcessFinished(a:vimPID, v:shell_error)
"   return [1, "finished :!"]
" endfun
" 
" fun! vl#lib#quickfix#runtaskinbackground#RunProcessInPythonThread(vimPID)
"   if !s:ClientServer()
"     return [0, "no client server"]
"   endif
"   if s:failPython
"     return [0, "python debug aborted"]
"   endif
"   if !has('python')
"     return [0, "this vim is not python enabled"]
"   endif
"   if !has('clientserver')
"     return [0, "no client server version"]
"   endif
...
"   endif
"   if has('win16') || has('win32') || has('win64')
"     return [0, "run task in background using python threads not yet supported"]
"   endif
"   call vl#lib#ide#logging#Log("executing command using python")
...
" let g:a = (g:processes[a:vimPID]['cmd'])
" py << EOF
...
"       self.executeVimCommand("call vl#lib#quickfix#runtaskinbackground#NotifyProcessId(%s,%s)"%(self.vimPID,popenobj.pid))
...
"       stdoutwriter.close()
"       popenobj.wait()
"       self.executeVimCommand("call vl#lib#quickfix#runtaskinbackground#ProcessFinished(%s,%d)"%(self.vimPID,popenobj.returncode))
"     except Exception, e:
"       self.executeVimCommand("call vl#lib#quickfix#runtaskinbackground#ProcessHasentBeenStartedBecause(%s,'%s')"%(self.vimPID,"exception: "+str(e)))
"     except:
"       # I hope command not found is the only error which might  occur here
"       self.executeVimCommand("call vl#lib#quickfix#runtaskinbackground#ProcessHasentBeenStartedBecause(%s,'%s')"%(self.vimPID,"command not foun"))
...
" 
" EOF
" end region 'file <dotvim>/autoload/vl/lib/quickfix/runtaskinbackground.vim'
...
" start region 'file <dotvim>/autoload/vl/lib/introspection.vim'
" "|fld   description : get vim path
" 
" " get the path of this vim instance (needs libc.so.5)
" " used in runtaskinbackground.vim
" " Luc proposed something like:
" fun! vl#lib#introspection#VimFilepath()
"   silent! let vimpid = libcallnr("libc.so.5", "getpid", "")
"   " matchstr is used to get rid of the newline char
"   if vimpid == 0
"     return [0, "libc.so.5 not found"]
"   else
"     let vim = substitute(system("pmap ".vimpid." | sed -n -e 's/.* //' -e '2p'"),"\n\\|\r",'','g') 
"     if executable(vim)
"       return [1,vim]
"     else
"       return [0, 'no vim executable found']
"     endif
"   endif
" endfunction
" end region 'file <dotvim>/autoload/vl/lib/introspection.vim'
